# Local Composer and Node Package Cache – Architecture

## 1. Goal and scope

- **Goal:** Re-run `composer install` and `npm install` (and optionally `pnpm install` / `yarn`) in a project without network access, by using a local cache and proxy. The cache is **machine-wide** (all projects on this machine), not tied to any single vendor or product.
...
- **Primary mechanism: local read-through proxy.** A local HTTP(S) service intercepts requests to packagist.org and registry.npmjs.org (and optionally other registries). On each request it tries the local cache first; on cache miss or when the cache is corrupted/unavailable it **fails over to the upstream registry**, fetches from the internet, stores the result in the cache, and returns it. No manual env vars are required for normal use once a project is configured to use the proxy (see per-project setup below).
- **Optional pre-populate:** A separate step can run `composer install` / `npm ci` against one or more project paths (with the proxy as registry) to seed the cache from lockfiles before going offline.
- **Minimize project impact:** Prefer project-local config (e.g. `.npmrc`, Composer `repos` in `composer.json` or config) over global env vars. When env or global config must change, **automate it** via scripts (e.g. a per-project setup script that writes the right config so the developer does not have to set env vars manually).
- **Failover:** If the local cache or proxy is corrupted or unavailable, clients must **naturally fall back to the real upstream registries** (packagist, npm registry) so that work can continue without the cache.
...
- **Inputs:** Path(s) to project directories that contain `composer.lock` and/or `package-lock.json` (or `pnpm-lock.yaml`).
- **Process:**
  ...
  - Run `composer install --no-install` (or equivalent) in each PHP project so Composer downloads all packages into cache; then run `composer install` to confirm.
  - Run `npm ci` (or `npm install`) in each Node project so npm downloads all tarballs into cache; similarly for pnpm if in scope.
...
- **Normal use:** After per-project setup (3.5), the project points at the local proxy. User runs normal `composer install` and `npm install` (or `pnpm install`); package managers talk to the proxy, which serves from cache or fails over to upstream. No env vars needed.
...
- Projects use the proxy via **per-project config** (see 3.5), not global env vars.

### 3.5 Per-project setup (scripted)

- **Goal:** Minimize project impact and ease reuse. Prefer project-local config over global env; when config is required, **automate it** so the developer does not have to set env vars or edit config by hand.
- **Script:** A command such as `pkg-cache setup-project /path/to/project` (or `pkg-cache setup-project` when run from inside the project) that:
  - Detects project type (Composer, npm, pnpm, or both).
  - Writes or updates project-local config so that installs use the local proxy:
    - **Composer:** e.g. `composer config repo.packagist composer https://127.0.0.1:<port>` so Composer uses the proxy without COMPOSER_HOME or other env.
    - **npm/pnpm:** Writes or updates `.npmrc` in the project root with `registry=https://127.0.0.1:<port>/` so npm install / pnpm install use the proxy without setting npm_config_cache or env.
  - Optionally backs up existing repo/registry settings and documents how to revert (for failover or use upstream only).
- **Reuse:** Running the same script on another machine (or in another clone) configures that project to use that machine's cache proxy, making reuse a single command per project.
